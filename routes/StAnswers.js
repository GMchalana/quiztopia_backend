...
router.post('/submit-answers-manual', moduleController.saveManualAnswers);

router.get('/review-manual/:attemptId', moduleController.getAttemptDetails);
...
router.get("/quiz-attempts/:userId", (req, res) => {
  ...
  const attemptsQuery = `
    ...
    WHERE qa.userId = ? AND m.manualOrAuto = 'auto'
...
router.get("/module-review/:userId/:moduleId/:attemptId", (req, res) => {
  ...
  router.get("/quiz-attempts-manual/:userId", (req, res) => {
    const userId = req.params.userId;
    ...
    const attemptsQuery = `
      ...
      FROM quizattempt qa
      INNER JOIN module m ON m.id = qa.moduleId
      WHERE qa.userId = ? AND m.manualOrAuto = 'manual'
      ORDER BY qa.createdAt DESC
    `;
